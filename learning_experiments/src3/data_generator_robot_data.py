    ...
    def generate_dataset(self, ignore=["unlabelled"], args=None):
        ...
        for folder_name in self.folder_names[:]:
            ...
            # for demonstration in demonstrations[:10]:
                ...
                train_indecies = np.random.choice(list(range(number_of_files)), train_n, replace=False)
                test_indecies = np.array([x for x in range(number_of_files) if x not in train_indecies])

                train_files = np.take(file_list_train, train_indecies)
                test_files = np.take(file_list_train, test_indecies)

                if "train" not in ignore:
                    for file_idx in tqdm(list(range(len(file_list_train)))):
                        ...
                        if self.mask_mode == 'loading':
                            ...
                            masks = list(dict_entry.values())
                            labels = list(dict_entry.keys())
