...
def plot_loss_curves(stats=None, args=None):
    ...
    for i, channel in enumerate(stats):
        if channel == "valid_label_loss" or channel == "valid_label_acc" or channel == "train_accs" or channel == "valid_label_acc_1" or channel == "valid_label_acc_2":
            continue
        plt.plot(list(range(len(stats[channel]))),stats[channel], color=colors[i], label=channel)
    ...
    plt.figure(figsize=(10, 10))
    plt.grid()
    plt.plot(list(range(len(stats['valid_label_loss']))),stats['valid_label_loss'], color='g', label='valid_label_loss')
    plt.xlabel("Epoch #", fontsize=14)
    plt.ylabel("Loss", fontsize=14)
    plt.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize=14)
    ...
    plt.figure(figsize=(10, 10))
    plt.grid()
    plt.plot(list(range(len(stats['valid_label_acc']))),stats['valid_label_acc'], color='r', label='valid_label_acc')
    plt.xlabel("Epoch #", fontsize=14)
    plt.ylabel("Loss", fontsize=14)
    plt.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize=14)
    ...
    plt.figure(figsize=(10, 10))
    plt.grid()
    plt.plot(list(range(len(stats['train_accs']))),stats['train_accs'], color='b', label='train_accs')
    plt.xlabel("Epoch #", fontsize=14)
    plt.ylabel("Loss", fontsize=14)
    plt.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize=14)
...
def attach_colors(labels=None, n_groups=None, composite=True):
    ...
    if "unknown" not in list(result["singular"].keys()):
...
def plot_overall_distribution(data=None, labels=None, groups=None, boundaries=None, colors=None, model=None, 
    ...
    if overlay:
        ...
        X, Y = np.meshgrid(x, y)
        Z = multivariate_normal.pdf(np.array([list(zip(c,d)) for c,d in zip(X,Y)]), mean=mean, cov=cov)
...
def plot_separate_distributions(data=None, labels=None, groups=None, boundaries=None, 
    ...
    # scatter plot all the data points in the latent space
    dimensions_pairs = list(itertools.combinations(list(range(model.n_latent)), 2))
...
def test_time_classification(data_test=None, data_all=None, labels=None, unseen_labels=None, groups=None, 
    ...
    for key in sorted(classifiers.keys()):
        ...
        classifier_tuples = list(zip(classifiers[key], classifiers[key][1:]))
    ...
    for index,key in enumerate(sorted(classifiers.keys())):
        for label in list(set(all_labels[index])):
            ...
            if label in groups[key] or label[0] == "u":
                ...
                filtered_data = chainer.Variable(np.repeat(data_all, len(list(groups.keys())), axis=0).take(indecies, axis=0))
    ...
    for key in sorted(classifiers.keys()):
        ...
        stds[key] = [sorted(point_stds, key=lambda k: k["value"])[0]["label"] for point_stds in stds[key]]
...
def label_analysis(labels=None, predictions=None, groups=None, model=None, folder_name=None, name=None):
    ...
    plot_confusion_matrix(cms=cms, group_classes=list(zip(true_sets, pred_sets)),
...
def plot_confusion_matrix(cms=None, group_classes=None,
    ...
    for i, subfig in enumerate(subfigures):
        ...
        subfig.set_xticks(list(range(len(pred))), minor=False)
        subfig.set_xticklabels(pred, minor=False, fontsize=30)
        subfig.set_yticks(list(range(len(true))), minor=False)
        subfig.set_yticklabels(true, minor=False, fontsize=30)

        fmt = '.2f'
        thresh = cm.max() / 2.
        for x, y in itertools.product(list(range(cm.shape[0])), list(range(cm.shape[1]))):
